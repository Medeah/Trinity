...
float *
fmmult (float s, float *A, int rowsA, int colsA)
{
  int i;
  float *resMatrix;
...
float *
mfdiv (float s, float *A, int rowsA, int colsA)
{
  int i;
  float *resMatrix;
  resMatrix = malloc (rowsA * colsA * sizeof (float));
...
float *
mmmult (float *A, int rowsA, int colsA, float *B, int rowsB, int colsB)
{
...
float
dotProduct (float *A, float *B, size_t size)
{
  float sum = 0.0f;
  int i;
...
bool
mmeq (float *A, float *B, size_t rows, size_t cols)
{
  int i;
...
float *
mmadd (float *A, float *B, int rows, int cols)
{
  int i;
  float *resMatrix;
...
float *
mmsubt (float *A, float *B, int rows, int cols)
{
  int i;
  float *resMatrix;
  resMatrix = malloc (rows * cols * sizeof (float));
...
main (void)
{
  float *_u0 = malloc (5 * sizeof (float));
  _u0[0] = 1;
  _u0[1] = 2;
  _u0[2] = 3;
  _u0[3] = 4;
  _u0[4] = 5;
  d = _u0;
  float *_u1 = malloc (5 * sizeof (float));
  _u1[0] = 1;
  _u1[1] = 2;
  _u1[2] = 3;
  _u1[3] = 4;
